...
# Day 33: support tickets. Path is relative to the repo containing agent-web/,
# same PROJECT_ROOT convention as everything else in this file — no separate env var.
_TICKETS_PATH = PROJECT_ROOT / "agent-web" / "data" / "support" / "tickets.json"
...
def list_tickets(status: str = "") -> str:
    """List support tickets (id, title, product_area, status). Optional `status` filter
    (e.g. 'resolved', 'open') — empty string returns all tickets. No search_tickets tool
    by design (day 33 scope): /support takes a ticket_id directly."""
    tickets = _load_tickets()
    if status:
        tickets = [t for t in tickets if t.get("status") == status]
    if not tickets:
        return "(no tickets found)"
    lines = [f"{t['id']}: {t['title']} [{t.get('status', '?')}]" for t in tickets]
    return "\n".join(lines)


@mcp.tool()
def get_ticket(id: str) -> str:
    """Get the full record (all fields, including history) for one support ticket by id,
    as JSON. Returns an error string if the id is unknown."""
    for t in _load_tickets():
        if t.get("id") == id:
            return json.dumps(t, ensure_ascii=False, indent=2)
    return f"Error: ticket '{id}' not found."
