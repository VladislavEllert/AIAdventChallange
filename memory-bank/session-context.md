...
## ⏩ ACTIVE (2026-07-18) — неделя 7, пишем план

**Сейчас:** фаза обсуждения дней 31–35 ЗАВЕРШЕНА. Следующий шаг — написать `/plan` на все 5 дней
сразу (Opus 4.8 medium). Код НЕ писать до явной отмашки.

- Вход для плана: **`week-07/PLAN-BRIEF.md`** (все договорённости) + лекция недели 7.
- Таргет: `agent-web/` (наш репо). Провайдер: ProxyAPI `openai/gpt-4o-mini`.
- Харнес подключён: `/plan /build /review /debug` (см. `AGENTS.md`).
- Стартовый промпт для новой сессии: `week-07/NEXT-SESSION-PROMPT.md`.
- Memory: `week07-target-project.md` (решения зафиксированы).
