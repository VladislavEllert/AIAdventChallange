...
0. **Clarify-gate (before any spawn).** Planner+skeptic spawns are expensive parallel background
   agents — a wrong premise burns a full round, and UI-referencing feature descriptions are the
   riskiest ("вкладка", "экран", "кнопка", "форма", any named screen/tab). If the one-line feature
   references a UI surface you have NOT verified against actual code/schema (or no screenshot was
   given), do ONE quick grep/read first to check the surface exists as described. If still
   ambiguous or you'd be guessing at the data model, STOP and ask the user (AskUserQuestion or a
   direct question) before spawning. Do not spend a planner+skeptic round on an unverified guess.
