...
**Always sweep the "active tasks" / current-focus file** (`tasks/active.md`,
`active.md`, `ACTIVE.md`, or whatever the bank calls its leading per-session
log). It is the **single biggest growth surface** — every session appends a new
"what I did today" block and nothing ever removes the old ones, so it silently
balloons to hundreds of lines and stops being a *current-state* file. Each defrag
run **must actualize AND compress it**, not just correct stray facts:

- It should read as **now**, not as an append-only session diary. Target a lean
  file (rule of thumb: current phase + open questions + TODO + live reference +
  the last 1–2 sessions in detail — the rest folded).
- **Collapse every closed session** (shipped feature, resolved bug, past meeting,
  completed deploy) into a one-line pointer to where the durable detail already
  lives — `progress.md`, `meetings/`, `swarm-report/`, an ADR. Don't keep the
  full narrative in two places; the active file keeps the pointer, the archive
  keeps the story.
- **Fold, don't lose** (same rule as everywhere): before deleting a session
  block, confirm each still-relevant fact it holds — an unanswered question, a
  pending TODO, a design/implementation constraint, a hard-won lesson — is either
  preserved compactly in the active file or already captured in its archive
  home. Struck-through/`✅ ~~resolved~~` items collapse to a short "closed:" line.
- Keep the file's existing top-of-file update protocol (the "update after each
  session" header, date convention) intact.
