...
| 07 | 35 | Ритуал курса: `rituals/day_report.py` — три роли обычными функциями (`collect()` git diff/status+метрики без LLM, `draft()` единственный LLM-вызов `openai/gpt-4o-mini`, `verify()` без LLM — формат таблицы/ссылка/anti-hallucination видео регуляркой) + `build_patch()` upsert строки. `tools/git_tools.py`: `git_diff`/`git_log`/`git_status` (safe, НЕ в общем реестре — коллизия имён с MCP `project_server.py` дня 31, реальный риск `400` от OpenAI на дубли имён функций) + `git_commit` (dangerous, стейджит только переданные пути, без `-A`); `git_push` не реализован вообще, не за флагом. `/ritual day <NN> [--dry-run]` реиспользует ВЕСЬ confirm/executor день 34 (`ConfirmToolModal`, новых UI-компонентов нет). `review/pipeline.py`: `PROMPT_V2_STRICT` (явный чеклист 5 реальных классов багов проекта) — recall 33%→100% на 4 фикстурах против `v1` в одну сессию/индекс (`review_eval/results_day32.md`, Прогоны 3-4), `run_eval.py` теперь дописывает секции вместо перезаписи. Живые баги, пойманные браузером/живым прогоном (не pytest): (1) наивный `diff[:N]` срез терял только что созданные файлы репо, не коммитившегося неделю — исправлено сортировкой untracked/added-first с бюджетом на файл; (2) модель дважды эхом вставляла заголовок таблицы перед строкой данных — верификатор поймал (`got 20` columns), добавлена defensive-постобработка `_extract_data_row()`; (3) confirm-модалка без ограничения высоты JSON-дампа аргументов — `write_file` с полным содержимым файла толкал кнопки Allow/Deny за пределы экрана, недостижимо и для живого пользователя, не только для Playwright — исправлено (`maxHeight`+`overflowY` на блоке аргументов). `pytest agent-web/tests`: **214 passed** (было 207, +1 после фикса ниже). Живой баг №4: `git_tools._git_commit` без pathspec на `git commit` коммитил ВЕСЬ индекс — застейдженный заранее `TOOLS_DOC_DRAFT.md` (день 34) незаметно попал в этот же коммит; исправлено (`git commit -m msg -- <paths>`), `tool-confirm.spec.ts` бейзлайн обновлён и день-34 сценарии перепрогнаны зелёными. Playwright `ritual-command.spec.ts`: 2/2 (dry-run diff в чате, confirm-модалка+Deny репо не тронут) — позитивный Allow-сценарий намеренно НЕ автоматизирован в Playwright (коммит README/progress.md на каждый CI re-run — плохой паттерн для боевых доков), сделан вручную через headless CLI после ревью диффа человеком. `grep -rn "git push"` в новом коде — ничего, кроме докстринга о его отсутствии. Вне скоупа (35.5, отдельное разрешение): 3 синтетических PR против живого GitHub Actions | done | [week-07/day-35](week-07/day-35/) | todo |
